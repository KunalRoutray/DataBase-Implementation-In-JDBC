...
public class lab3_task4 {
    ...
    public static void main(String[] args) {
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            System.out.println("=== BEFORE TRANSACTION (SCENARIO 1) ===");
            showBalances(conn);
            ...
            System.out.println("=== AFTER TRANSACTION (SCENARIO 1) ===");
            showBalances(conn);
    ...
    private static void setupDatabase(Connection conn) throws SQLException {
        try (Statement stmt = conn.createStatement()) {
            stmt.execute("CREATE TABLE Accounts1111 ("
                + "acct_id INT PRIMARY KEY, "
                + "owner VARCHAR(50), "
                + "balance DECIMAL(10,2) NOT NULL CHECK (balance >= 0))");

            // Insert only if empty
            try (ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM Accounts1111")) {
                rs.next();
                if (rs.getInt(1) == 0) {
                    stmt.execute("INSERT INTO Accounts1111 VALUES (101, 'Alice', 1200.00)");
                    stmt.execute("INSERT INTO Accounts1111 VALUES (102, 'Bob', 450.00)");
                    stmt.execute("INSERT INTO Accounts1111 VALUES (103, 'Charlie', 780.00)");
                    stmt.execute("INSERT INTO Accounts1111 VALUES (104, 'David', 920.00)");
                    stmt.execute("INSERT INTO Accounts1111 VALUES (105, 'Eve', 610.00)");
    ...
    private static void resetBalances(Connection conn) throws SQLException {
        try (Statement stmt = conn.createStatement()) {
            stmt.execute("UPDATE Accounts1111 SET balance = 1200.00 WHERE acct_id = 101");
            stmt.execute("UPDATE Accounts1111 SET balance = 450.00 WHERE acct_id = 102");
            stmt.execute("UPDATE Accounts1111 SET balance = 780.00 WHERE acct_id = 103");
            stmt.execute("UPDATE Accounts1111 SET balance = 920.00 WHERE acct_id = 104");
            stmt.execute("UPDATE Accounts1111 SET balance = 610.00 WHERE acct_id = 105");
        }
    ...
    private static void transferFunds(Connection conn, int fromAcct, int toAcct,
        ...
        try (Statement stmt = conn.createStatement()) {
            stmt.executeUpdate(
                "UPDATE Accounts1111 SET balance = balance - " + amount +
    ...
    private static void showBalances(Connection conn) throws SQLException {
        ...
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(
                 "SELECT owner, acct_id, balance FROM Accounts1111 ORDER BY acct_id")) {
            while (rs.next()) {
